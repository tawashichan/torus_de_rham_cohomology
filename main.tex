...
\]
を考える。これらはそれぞれトーラスの横方向・縦方向を一周する閉曲線である。
...
\subsection{\(\alpha,\beta\) の基本的な閉曲線に沿う積分}
...
以下の証明では、被覆空間の deck transformation を用いて
\(g\) のずれが period と一致することを示す。
座標計算による証明は付録に回す。
\end{remark}
...
\tau_{m,n}(x,y)=(x+m,y+n)
\]
とおく。これは \(\pi\) の deck transformation なので
\(\pi\circ\tau_{m,n}=\pi\) である。したがって
\[
d(g\circ\tau_{m,n}-g)
=
\tau_{m,n}^*(dg)-dg
...
0.
\]
\(\R^2\) は連結なので、\(g\circ\tau_{m,n}-g\) は定数関数である。
この定数を \(c_{m,n}\in\R\) と書く。すなわち
\[
...
まず \((m,n)=(1,0)\) の場合を考える。上の左辺は定数だから
\((x,y)=(0,0)\) を代入して
\[
c_{1,0}=g(1,0)-g(0,0)
\]
を得る。一方、\(\widetilde{\gamma}_x(t)=(t,0)\) だから
\[
c_{1,0}
=
g(1,0)-g(0,0)
...
\int_{\gamma_x}\omega.
\]
\(\Phi([\omega])=(0,0)\) より \(\int_{\gamma_x}\omega=0\) なので
\[
c_{1,0}=0.
\]

同様に、\(\widetilde{\gamma}_y(t)=(0,t)\) を用いると
\[
c_{0,1}
=
g(0,1)-g(0,0)
...
さらに、定数 \(c_{m,n}\) は \((m,n)\) に関して加法的である。実際、
\(\tau_{m+m',\,n+n'}=\tau_{m,n}\circ\tau_{m',n'}\) なので
\[
g\circ\tau_{m+m',\,n+n'}-g
=
(g\circ\tau_{m,n}-g)\circ\tau_{m',n'}
+
(g\circ\tau_{m',n'}-g).
\]
右辺は定数関数 \(c_{m,n}+c_{m',n'}\) だから
\[
c_{m+m',\,n+n'}
=
c_{m,n}+c_{m',n'}.
...
\(0\) 次の場合の不変関数との対応より、ある \(f\in C^\infty(T^2)\) が存在して
\[
\pi^*f=g
\]
を満たす。
...
ここでは、横方向・縦方向を一周する2つの閉曲線に沿う積分が、
閉 \(1\)-形式のコホモロジー類を検出していると見ることができる。
\end{remark}
...
\section*{補論：境界である \(1\)-サイクルに沿う閉 \(1\)-形式の積分}

\begin{prop}
\(\omega\in\Omega^1(T^2)\) を閉 \(1\)-形式とし、
\(\gamma\) を閉曲線、すなわち特異 \(1\)-サイクルとする。

さらに \(\gamma\) がホモロジー的に自明、すなわちある滑らかな特異 \(2\)-chain \(C\) に対して
\[
\partial C=\gamma
\]
となるとする。このとき
...
つまり閉 \(1\)-形式の積分は、閉曲線そのものではなく
\[
[\gamma]\in H_1(T^2;\R)
\]
というホモロジー類だけに依存する。特に \(\gamma\) が境界なら、その積分は \(0\) である。
\end{remark}
...
したがって、基本的な閉曲線に沿う period がすべて \(0\) なら、
\(g\) は \(\Z^2\)-不変、すなわち \(T^2\) 上の関数に降りる。
本文の \(H^1_{\mathrm{dR}}(T^2)\) の計算では、この事実を用いて
\(\Phi\) の単射性を示した。
\end{remark}

\section*{付録：\(\Phi\) の単射性の座標計算}

本文では deck transformation を用いて \(\Phi\) の単射性を示した。
ここでは同じ点、すなわち \(dg=\pi^*\omega\) を満たす原始関数 \(g\) の
\(\Z^2\)-周期性を、座標計算で直接確認する。
...
一方、\(\int_{\gamma_x}\omega=0\) と仮定しているので、\((2)\) と \((C1)\) を用いて
\[
...
\((C3)\) より \(\partial g/\partial x=A\) であるから
\[
g(x+1,y)-g(x,y)
=
\int_x^{x+1} A(s,y)\,ds.
...
\((C3)\) より \(\partial g/\partial y=B\) であるから
\[
g(x,y+1)-g(x,y)
=
\int_y^{y+1} B(x,t)\,dt
...
\pi^*(df)=d(\pi^*f)=dg=\pi^*\omega.
\]
\(\pi^*\) の単射性より \(df=\omega\) である。
したがって \(\omega\) は exact であり、\(\ker\Phi=0\) が従う。
\end{proof}
