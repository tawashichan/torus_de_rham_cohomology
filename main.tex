...
% todo \pi^*がZ^2周期のR^2上の形式とT^2上の形式の同型を与えていることを明示的に書いた方が良い
上の不変性により、\(dx,dy,dx\wedge dy\) は \(T^2\) 上の形式に降りる。
それらを
\[
...
% 方針 R^2は可縮より任意の閉形式はexactである。\omegaを\piで引き戻してR^2における形式にすると外微分と引き戻しの可換性から\pi^*\omegaは閉形式である。よって\pi^*\omegaはexactである。つまりdg=\pi^*\omegaなるgが存在する。このgはZ^2周期を持つことが示せる。よって\pi^*f=gなるfが存在する。このfについて\omega=dfが成り立つことが示せる。

% todo 積分写像\Phiの単射性を示す際にKer\Phi={0}を証明する方針をとる(つまりKernelにはexactな形式しかいないことを示す)が、formがKer\Phiに属していること(つまり積分して0)は、R^2側でgがZ^2周期的であることを示すことのみに用いられている。これはどういうことか説明できるようにする。
% ↑デッキ変換を使うとg\circ\tau_{m,n}−gが定数になることがわかる。積分して0という仮定からこの定数が0であることが示せる。
